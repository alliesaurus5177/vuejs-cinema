<template>
	<div id="day-select">
		<ul class="days">
			<li v-bind:class="{ day: true, active: isActive(day) }" v-for="day in days" v-on:click="selectDay(day)">{{ formatDay(day) }}
			</li>
...
<script>
	export default{
		...
		data() {
			return {
				//selected: this.$moment(), set selected automatically to today - now a prop see above
		...
		methods: {
			...
			changeDay(change) {
				let newDay = this.$moment(this.selected).add(change, 'days'); //add or subtract one day off 
